...
---------------------------------------------------*/
	...
	// Fix hero orange circle
	$(document).ready(function() {
		// Use setTimeout to ensure this runs after all other scripts
		setTimeout(function() {
			// Force the primary color to purple
			$('body').attr('data-primary-color', '#9900ff');
			
			// Apply hue-rotate filter to orange elements
			$('.hero-translate, #hero-bg-image, .hero-video-wrapper, #hero-interaction img').css('filter', 'hue-rotate(265deg)');
			
			// Override any orange styles
	...
	// Disable AJAX navigation to fix CORS errors
	function FirstLoad() {	
	    ...
	    // Comment out AJAX navigation initialization
	    /*
		if( $('.ajax-links-float').length > 0 ){
			//Floating Line Animation
			FloatingLinesNavigation();	
		}
	    
		// Page Navigation Events
		if ($(".clone-header").hasClass("header-clone")){
			$(".ajax-link, .ajax-link-menu, .ajax-link-footer, #nav-scroll-down").on('click', function() {
	            // ...existing code...
			});
	    }
	    */
	    
	    // Replace AJAX navigation with regular navigation
