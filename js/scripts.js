jQuery(function ($) {
	...
	$(document).ready(function() {
		...
		// Force purple color for all elements
		$('body').attr('data-primary-color', '#9900ff');
		$('.hero-video-wrapper').css('background-color', '#9900ff');
		$('.hero-translate').css('background-color', '#9900ff');
		
		// For hero section specifically
		$('#hero-bg-image').css('filter', 'hue-rotate(265deg)');
		$('.hero-video-wrapper').css('filter', 'hue-rotate(265deg)');
		
		// For any inline styles with orange color
		$('[style*="#fa821d"]').each(function() {
			let style = $(this).attr('style');
			if(style) {
				style = style.replace(/#fa821d/g, '#9900ff');
				$(this).attr('style', style);
			}
		});
		
		// Fix CORS errors by disabling AJAX navigation completely
		$(".ajax-link, .ajax-link-menu, .ajax-link-footer").each(function() {
			$(this).removeClass("ajax-link ajax-link-menu ajax-link-footer");
			$(this).removeAttr("data-type");
		});
		
		// Ensure active class is preserved for current page
		...
		// Fix for all remaining AJAX navigation links
		$("a[data-type='page-transition']").each(function() {
			var href = $(this).attr("href");
			$(this).removeAttr("data-type");
			$(this).removeClass("ajax-link ajax-link-menu ajax-link-footer next-ajax-link-page");
		});
		
		// Fix any showcase slides
		$(".thumb-slide a, .slide-link, a.slide-link").each(function() {
			$(this).removeAttr("data-type");
			$(this).removeClass("slide-link");
		});
		
		// Make sure all links work normally
		$("a").on("click", function(e) {
			// Only prevent default if it has href="#"
			if ($(this).attr("href") === "#") {
				e.preventDefault();
			}
		});
		
		// Fix the "Back Top" button
		$("#backtotop").on("click", function(e) {
			e.preventDefault();
			$("html, body").animate({ scrollTop: 0 }, 800);
		});
		
		// Override AJAX page transition functions
		function overrideAJAXFunctions() {
			// Disable any AJAX navigation functions
			if (window.FirstLoad) {
				const originalFirstLoad = window.FirstLoad;
				window.FirstLoad = function() {
					// Call original but disable AJAX navigation part
					try { originalFirstLoad(); } catch(e) { console.log("Error calling original FirstLoad", e); }
					
					// Disable AJAX navigation components
					disableAJAXNavigation();
				};
			}
			
			function disableAJAXNavigation() {
				// Remove AJAX classes from all links
				$("a").each(function() {
					$(this).removeAttr("data-type");
					$(this).removeClass("ajax-link ajax-link-menu ajax-link-footer next-ajax-link-page slide-link");
				});
				
				// Unbind any AJAX click handlers
				$("a").off("click.ajax");
			}
		}
		
		// Call the override function
		setTimeout(overrideAJAXFunctions, 500);
	});
